// 저장 구조 v7 → v8(무드보드 카드 그룹화 groupId 추가) 마이그레이션이
// 무사한지 확인하는 테스트입니다.
//
// 왜 마이그레이션에 테스트가 반드시 필요한지는
// test/data/migration_v1_to_v2_test.dart 맨 위 설명을 보세요. 같은 이유입니다.
//
// 이번 마이그레이션은 board_cards 표에 group_id 칼럼을 addColumn으로
// 추가하는 것뿐이지만, test/data/migration_v4_to_v5_test.dart(boards.folderId)와
// **똑같은 함정**이 있습니다 — v2→v3 단계의 createTable(boardCards)는
// "그 당시의" 표 모양이 아니라 **지금 이 순간의 tables.dart**로 표를
// 만듭니다. 그래서 v1이나 v2에서 곧장 v8로 건너뛰는 사용자는 v3 단계에서
// 이미 group_id가 포함된 board_cards 표를 받고, 그 뒤 v8 단계가 또
// addColumn을 하려다 "칼럼이 이미 있다"는 오류로 앱이 아예 안 켜집니다.
// `onUpgrade`의 v8 조건을 `from >= 3 && from < 8`로 맞춰서 막았습니다 —
// 이 테스트의 "v1에서 v8로 한 번에 건너뛰어도" 케이스가 정확히 그
// 버그를 잡습니다.
...
void main() {
  ...
  ///
  /// [version]에 7을 넘기면 무드보드 카드 표까지 있지만 group_id는 아직
  /// 없는 v7 모습, 1을 넘기면 파트도 무드보드도 없던 v1 모습입니다.
  /// [cardId]를 주면 그 번호로 카드 하나를 미리 넣어둡니다(version이
  /// 3 이상일 때만 뜻이 있습니다).
  void createOldDatabase({required int version, String? cardId}) {
    ...
    final String parentIdColumn = version >= 7 ? 'parent_id TEXT,' : '';
    ...
    // v3부터 무드보드 표 두 개가 있습니다. v7까지는 board_cards에
    // group_id가 없습니다 — 이번에 새로 추가하는 칼럼이기 때문입니다.
    if (version >= 3) {
      raw.execute('''
        CREATE TABLE boards (
          id TEXT NOT NULL,
  ...
  test('앱이 켜지고, 있던 카드가 안 사라진다', () async {
    createOldDatabase(version: 7, cardId: 'card-1');
  ...
  test('예전 카드는 groupId가 비어 있다', () async {
    createOldDatabase(version: 7, cardId: 'card-1');
  ...
  test('마이그레이션 뒤에는 그룹으로 묶어서 저장할 수 있다', () async {
    createOldDatabase(version: 7, cardId: 'card-1');
    ...
    final BoardCard card = (await repository.getCards('board-1')).first;

    await repository.saveCard(card.copyWith(groupId: 'group-1'));

    final BoardCard updated = (await repository.getCards('board-1')).first;
  ...
  test('두 번 열어도 오류 없이 그대로다', () async {
    ...
    // 두 번째로 열 때는 이미 schemaVersion이 8이라 addColumn이 다시
  ...
  test('v7에서 v8로만 건너뛰어도 addColumn이 한 번만 실행된다', () async {
    // 이 테스트가 실제로 확인하는 것: board_cards 표가 **이미 group_id
    // 없이 실제로 존재하던** 사용자(v3~v7)는 addColumn이 정상적으로
    // 실행돼야 합니다. 아래 "v1에서 v8로" 테스트와 짝을 이룹니다 —
    // 그쪽은 반대로 addColumn을 **건너뛰어야** 하는 경우입니다.
    createOldDatabase(version: 7, cardId: 'card-1');
    ...
    expect(cards, isNotEmpty, reason: 'addColumn이 실패하면 앱 자체가 안 켜집니다');
  });

  test('v1에서 v8로 한 번에 건너뛰어도 오류 없이 켜진다', () async {
    // ── 실제로 겪은 버그(v5)와 같은 종류를 잡는 테스트입니다 ──
    // v2→v3 단계의 createTable(boardCards)는 "지금 이 순간의"
    // tables.dart로 표를 만들어서, 이미 group_id가 포함된 채로
    // board_cards 표가 생깁니다. 그 뒤 v8 단계가 또 addColumn을
    // 하려고 하면 "칼럼이 이미 있다"는 오류로 앱이 아예 안 켜집니다.
    // 한참 업데이트를 안 한 사용자는 정확히 이 경로(v1 → v8 직행)를
    // 지나가므로 반드시 확인해야 합니다.
    ...
    // 앱이 켜졌다는 것 자체가 이 테스트의 핵심입니다. 새 무드보드와
    // 카드를 만들어 groupId까지 정상적으로 쓸 수 있는지도 함께 봅니다.
    final LocalBoardRepository repository = LocalBoardRepository(db);
    ...
    expect(saved.first.groupId, 'group-1');
  });
}
